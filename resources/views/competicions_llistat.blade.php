...
@foreach($leaguesList as $league)
    ...
    @if($lastSeason !== $league->seasonName)
        ...
        <div class="season-section mb-10" data-season="{{ $league->seasonName }}">
            <div class="flex items-center gap-2 pb-2 border-b-2 border-stone-900 dark:border-[#1c1917] mb-6">
                <i class="fa-solid fa-trophy text-stone-900 dark:text-white text-lg"></i>
    ...
    <div class="bg-white dark:bg-[#121215] border border-stone-200 dark:border-stone-800 p-4 rounded-2xl shadow-xs hover:border-[#ffcc00] dark:hover:border-stone-600 hover:shadow-sm transition-all group league-item flex items-center justify-between" data-label="{{ strtolower($league->label) }}">
        ...
        <i class="fa-solid fa-chevron-right text-[10px] text-stone-400 group-hover:text-stone-900 dark:hover:text-white group-hover:translate-x-0.5 transition-all flex-shrink-0"></i>
    </div>
