...
<div class="w-full mt-2 mb-6 font-display">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-stone-200 dark:border-stone-800 pb-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('dashboard.anuncis.new') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-primary text-primary-text hover:bg-primary-hover dark:bg-stone-800 dark:text-white dark:hover:bg-stone-700 font-black text-xs uppercase tracking-wider rounded-full transition-all shadow-xs active:scale-95">
...
@if($anuncis->isEmpty())
    <div class="bg-white dark:bg-[#121215] border border-stone-200 dark:border-stone-800/90 rounded-3xl p-12 text-center shadow-xs mb-8 font-display">
        ...
        <a href="{{ route('anuncis.index') }}" class="mt-4 inline-flex items-center gap-1 px-4 py-2 rounded-full bg-primary text-primary-text hover:bg-primary-hover dark:bg-stone-800 dark:text-white font-black text-xs uppercase tracking-wider transition-all">
...
@else
    ...
    <div class="flex justify-center items-center gap-2 py-4 font-display">
        ...
        @else
            <a href="{{ $anuncis->previousPageUrl() }}" class="w-9 h-9 rounded-full bg-stone-100 dark:bg-stone-900 hover:bg-primary hover:text-primary-text dark:hover:bg-primary dark:hover:text-primary-text text-stone-800 dark:text-stone-200 flex items-center justify-center text-xs font-black transition-all border border-stone-200/80 dark:border-stone-800">
                <i class="fa-solid fa-chevron-left"></i>
            </a>
        @endif
        ...
        @foreach($anuncis->getUrlRange(1, $anuncis->lastPage()) as $page => $url)
            @if($page == $anuncis->currentPage())
                <span class="w-9 h-9 rounded-full bg-primary text-primary-text dark:bg-stone-800 dark:text-white flex items-center justify-center text-xs font-black shadow-xs">
                    {{ $page }}
                </span>
            @elseif($page == 1 || $page == $anuncis->lastPage() || abs($page - $anuncis->currentPage()) <= 2)
                <a href="{{ $url }}" class="w-9 h-9 rounded-full bg-stone-100 dark:bg-stone-900 hover:bg-primary hover:text-primary-text dark:hover:bg-primary dark:hover:text-primary-text text-stone-800 dark:text-stone-200 flex items-center justify-center text-xs font-black transition-all border border-stone-200/80 dark:border-stone-800">
                    {{ $page }}
                </a>
        ...
        @if($anuncis->hasMorePages())
            <a href="{{ $anuncis->nextPageUrl() }}" class="w-9 h-9 rounded-full bg-stone-100 dark:bg-stone-900 hover:bg-primary hover:text-primary-text dark:hover:bg-primary dark:hover:text-primary-text text-stone-800 dark:text-stone-200 flex items-center justify-center text-xs font-black transition-all border border-stone-200/80 dark:border-stone-800">
...
.filter-btn-active { background-color: var(--color-primary); color: var(--color-primary-text); border-color: var(--color-primary); }
.dark .filter-btn-active { background-color: var(--color-primary); color: var(--color-primary-text); border-color: var(--color-primary); }
