...
<nav class="sticky top-0 z-40 flex items-center justify-between py-3.5 px-4 md:px-6 border-b border-stone-200 dark:border-stone-800/80 mb-6 bg-white/90 dark:bg-[#09090b]/90 backdrop-blur-xl shadow-xs">
    ...
    <div class="hidden lg:flex items-center gap-6">
        <ul class="flex items-center gap-6 font-display text-sm font-bold text-stone-700 dark:text-stone-300">
            ...
            <li>
                <a href="/noticies" class="py-1.5 hover:text-stone-900 dark:hover:text-white transition-colors {{ request()->is('noticies*') ? 'border-b-2 border-stone-900 dark:border-[#1c1917] text-stone-900 dark:text-white' : '' }}">
                    Notícies
                </a>
            </li>
            <li>
                <a href="/pavellons" class="py-1.5 hover:text-stone-900 dark:hover:text-white transition-colors {{ request()->is('pavellons*') ? 'border-b-2 border-stone-900 dark:border-[#1c1917] text-stone-900 dark:text-white' : '' }}">
                    Pavellons
                </a>
            </li>
            <li>
    ...
    <div class="flex items-center gap-2 md:gap-4">
        ...
        <!-- Login / User Action -->
        @if (!Auth::check())
            <a href="/login" class="text-xs md:text-sm font-extrabold text-stone-700 dark:text-stone-300 hover:text-stone-900 dark:hover:text-white transition-colors">
                Login
            </a>
        @else
        ...
        searchTimeout = setTimeout(function() {
            ...
            Promise.all([teamsFetch, playersFetch])
                .then(([teamsData, playersData]) => {
                    ...
                    searchReturn.insertAdjacentHTML('beforeend', "<div class='block w-full mx-2 my-3 font-black text-stone-900 dark:text-white text-xs uppercase tracking-wider'>" + teamsData.length + " equips trobats</div>");
