...
@if ($type === 'ticker')
    <!-- TICKER STYLE (Hallmark High-Contrast Scoreboard Bar) -->
    <div class="inline-flex flex-col bg-white dark:bg-[#131419] border border-stone-300 dark:border-stone-800/90 rounded-lg p-2.5 shadow-sm mr-3 flex-shrink-0 w-64 text-xs transition-all hover:border-[#f5c310]">
        <div class="font-mono text-[9px] font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest mb-1 truncate">
            {{ $match->groupName }}
        </div>
        <div class="flex items-center justify-between font-mono">
            <div class="flex items-center gap-1.5 truncate max-w-[78%]">
                <span class="truncate font-medium text-stone-800 dark:text-stone-200" title="{{ App\Http\Controllers\TeamsController::teamFormat($match->localTeam) }}">{{ App\Http\Controllers\TeamsController::teamFormat($match->localTeam) }}</span>
                <span class="font-black text-[#f5c310] px-1 bg-stone-900 dark:bg-black rounded text-[11px]">{{ $match->localResult }} - {{ $match->visitorResult }}</span>
                <span class="truncate font-medium text-stone-800 dark:text-stone-200" title="{{ App\Http\Controllers\TeamsController::teamFormat($match->visitorTeam) }}">{{ App\Http\Controllers\TeamsController::teamFormat($match->visitorTeam) }}</span>
            </div>
            <!-- LIVE/FI Status Stamp -->
            <div class="flex items-center gap-1 flex-shrink-0">
                @if(isset($match->localResult))
                    <span class="font-mono text-[9px] font-bold text-stone-500 dark:text-stone-400 bg-stone-100 dark:bg-stone-800 px-1 py-0.5 rounded">FI</span>
                @else
                    <span class="font-mono text-[9px] font-extrabold text-black bg-[#f5c310] px-1 py-0.5 rounded animate-pulse">LIVE</span>
                @endif
            </div>
        </div>
    </div>
...
@elseif ($type === 'result')
    <!-- RESULT STYLE (Hallmark Scoreboard Card) -->
    @php
    ...
    @endphp
    <div class="bg-white dark:bg-[#131419] border border-stone-200 dark:border-stone-800/90 rounded-xl mb-3.5 overflow-hidden shadow-sm hover:border-[#f5c310]/80 transition-all p-3.5">
        <!-- Top header info -->
        <div class="font-mono text-[9.5px] font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest mb-2.5 truncate" title="{{ $match->groupName }}">
        ...
        <div class="flex items-center justify-between mb-3 gap-2">
            ...
            <!-- Local Logo -->
            <div class="w-7 h-7 bg-white rounded-md p-0.5 flex-shrink-0 flex items-center justify-center border border-stone-200 dark:border-stone-700 shadow-sm">
            ...
            <!-- Score Box -->
            <div class="flex items-center justify-center flex-shrink-0 bg-stone-900 dark:bg-black px-2.5 py-1 rounded border border-stone-800 shadow-inner">
                <span class="font-mono text-sm md:text-base font-black text-[#f5c310] tracking-tight">
        ...
        <div class="mt-2.5 pt-2 border-t border-stone-200 dark:border-stone-800/80 font-mono text-[10px] text-stone-500 dark:text-stone-400 flex justify-between items-center gap-2">
...
@else
    <!-- UPCOMING STYLE (Hallmark Agenda Fixture Card) -->
    <div class="bg-white dark:bg-[#131419] border border-stone-200 dark:border-stone-800/90 rounded-xl mb-3.5 overflow-hidden shadow-sm hover:border-[#f5c310]/80 transition-all p-3.5">
        <!-- Top header info -->
        <div class="font-mono text-[9.5px] font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest mb-2.5 truncate" title="{{ $match->groupName }}">
            {{ $match->groupName }}
        ...
        <div class="flex items-center justify-between mb-3 gap-2">
            ...
            <!-- Mid Time & Date Stamp -->
            <div class="flex flex-col items-center justify-center flex-shrink-0 bg-stone-100 dark:bg-stone-850 px-2 py-1 rounded border border-stone-200 dark:border-stone-800 min-w-[3.5rem] text-center">
                <span class="font-mono text-xs md:text-sm font-black text-[#f5c310] leading-none">
                    {{ \Carbon\Carbon::parse($match->matchHour)->format('H:i') }}
                </span>
                <span class="font-mono text-[8.5px] font-extrabold text-stone-500 dark:text-stone-400 mt-0.5 uppercase tracking-wider">
        ...
        @if($match->idMatch > 1000000)
            <div class="mt-2.5 pt-2 border-t border-stone-200 dark:border-stone-800/80">
                <div id="predict_{{ $match->idMatch }}" class="font-mono text-[10px] text-stone-400 text-center">
                    Calculant predicció...
                </div>
            </div>
        @endif
